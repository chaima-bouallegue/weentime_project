...
public interface CongeRepository extends JpaRepository<Conge, Long> {
       ...
       List<Conge> findByEntrepriseIdAndStatutOrderByDateCreationDesc(Long entrepriseId,
                     com.weentime.weentimeapp.enums.StatutDemandeEnum statut);
       ...
       @Query("SELECT c FROM Conge c WHERE c.entrepriseId = :entrepriseId AND c.statut = com.weentime.weentimeapp.enums.StatutDemandeEnum.APPROUVE "
                     +
                     "AND NOT (c.dateFin < :debut OR c.dateDebut > :fin)")
       List<Conge> findApprovedForDateRange(@Param("entrepriseId") Long entrepriseId, @Param("debut") LocalDate debut,
                     @Param("fin") LocalDate fin);
       ...
       java.util.Optional<Conge> findApprovedForUserAndDate(@Param("userId") Long userId,
       ...
       @Query("SELECT c FROM Conge c WHERE c.entrepriseId = :entrepriseId AND c.utilisateurId IN :ids AND c.statut = com.weentime.weentimeapp.enums.StatutDemandeEnum.APPROUVE "
                     +
                     "AND NOT (c.dateFin < :debut OR c.dateDebut > :fin)")
       List<Conge> findApprovedForUsersAndDateRange(@Param("entrepriseId") Long entrepriseId,
                     @Param("ids") List<Long> ids, @Param("debut") LocalDate debut, @Param("fin") LocalDate fin);
}
