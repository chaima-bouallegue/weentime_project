...
public class EquipeServiceImpl implements EquipeService {
    ...
    public EquipeResponse createEquipe(EquipeRequest request) {
        if (equipeRepository.existsByNomAndDepartementId(request.getNom(), request.getDepartementId())) {
            throw new IllegalArgumentException("Une équipe avec ce nom existe déjà dans ce département : " + request.getNom());
        }

        Departement departement = departementRepository.findById(request.getDepartementId())
                .orElseThrow(() -> new EntityNotFoundException("Departement non trouvé avec l'id : " + request.getDepartementId()));
        ...
        if (request.getResponsableId() != null) {
            ...
            boolean isManager = responsable.getRoles() != null && responsable.getRoles().stream()
                    .anyMatch(role -> role.getNom() == com.weentime.weentimeproject.enums.RoleNom.ROLE_MANAGER);
    ...
    public EquipeResponse updateEquipe(Long id, EquipeRequest request) {
        ...
        if (request.getDepartementId() != null) {
            Departement departement = departementRepository.findById(request.getDepartementId())
                    .orElseThrow(() -> new EntityNotFoundException("Departement non trouvÃ© avec l'id : " + request.getDepartementId()));
            equipe.setDepartement(departement);
        }
        if (request.getResponsableId() != null) {
            Utilisateur responsable = utilisateurRepository.findById(request.getResponsableId())
                    .orElseThrow(() -> new EntityNotFoundException("Utilisateur non trouvÃ© avec l'id : " + request.getResponsableId()));
            boolean isManager = responsable.getRoles() != null && responsable.getRoles().stream()
                    .anyMatch(role -> role.getNom() == RoleNom.ROLE_MANAGER);
            if (!isManager) {
                throw new IllegalArgumentException("Le responsable doit Ãªtre un MANAGER");
            }
            equipe.setResponsable(responsable);
        } else {
        ...
        return utilisateurRepository.findByEquipeId(id, pageable)
                .map(user -> {
                    ...
                    member.put("roles", user.getRoles() == null
                            ? java.util.List.of()
                            : user.getRoles().stream().map(role -> role.getNom().name()).toList());
