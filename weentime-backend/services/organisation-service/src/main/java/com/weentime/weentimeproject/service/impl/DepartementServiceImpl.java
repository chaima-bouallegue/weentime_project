...
public class DepartementServiceImpl implements DepartementService {
    ...
    public DepartementResponse createDepartement(DepartementRequest request) {
        if (departementRepository.existsByNomIgnoreCaseAndEntreprise_Id(request.getNom(), request.getEntrepriseId())) {
            throw new IllegalStateException("Un departement avec ce nom existe deja dans cette entreprise.");
        }

        if (departementRepository.existsByCodeInterneAndEntrepriseId(request.getCodeInterne(), request.getEntrepriseId())) {
    ...
    public DepartementResponse updateDepartement(Long id, DepartementRequest request) {
        ...
        if (!departement.getNom().equalsIgnoreCase(request.getNom())
                && departementRepository.existsByNomIgnoreCaseAndEntreprise_Id(request.getNom(), departement.getEntreprise().getId())) {
            throw new IllegalStateException("Un departement avec ce nom existe deja dans cette entreprise.");
        }

        if (!departement.getCodeInterne().equals(request.getCodeInterne())
                && departementRepository.existsByCodeInterneAndEntrepriseId(request.getCodeInterne(), departement.getEntreprise().getId())) {
            throw new IllegalStateException("Le code interne de ce departement existe deja dans cette entreprise.");
        }
