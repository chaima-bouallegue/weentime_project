...
@Component({
  ...
  template: `
    <ui-dashboard-layout
      [title]="(data()?.heroTitle || 'Pilotage RH')"
      [subtitle]="(data()?.heroSubtitle || 'Suivi de la présence et des validations RH')"
      [roleBadge]="(data()?.roleBadge || 'Ressources humaines')"
      ...
      <div layout-primary>
        @if (loading() && data()) {
          <div class="inline-loading"><ui-spinner [size]="18"></ui-spinner><span>Mise à jour RH...</span></div>
        }
        @if (hasPartialData()) {
        ...
        <ui-activity-feed
          [title]="'Activité RH'"
          [subtitle]="'Historique des opérations RH'"
      ...
      <div layout-side>
        ...
        <ui-quick-actions
          [title]="'Actions RH'"
          [subtitle]="'Validation et opérations'"
          [actions]="data()?.quickActions || []">
        </ui-quick-actions>

        <ui-notifications-panel
          [title]="'Notifications RH'"
          [subtitle]="'Alertes et demandes'"
      ...
      <div layout-bottom>
        ...
        <section class="panel">
          <h3>État du jour collaborateurs</h3>
          @if ((data()?.people || []).length === 0) {
            <p class="empty">Aucune entreprise affectée à ce compte RH.</p>
          } @else {
