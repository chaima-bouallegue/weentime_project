...
@Component({
  selector: 'ui-manager-dashboard-page',
  standalone: true,
  imports: [
    CommonModule,
  ...
  template: `
    <ui-dashboard-layout
      [title]="(data()?.heroTitle || 'Pilotage équipe')"
      [subtitle]="(data()?.heroSubtitle || 'Suivi des présences et validations manager')"
      [roleBadge]="(data()?.roleBadge || 'Manager')"
      ...
      <div layout-primary>
        @if (loading() && data()) {
          <div class="inline-loading"><ui-spinner [size]="18"></ui-spinner><span>Mise à jour équipe...</span></div>
        }
        @if (hasPartialData()) {
        ...
        <ui-analytics-layout>
          ...
          <div analytics-side class="panel">
            ...
            <ui-progress-ring [label]="'Couverture'" [value]="presentSegment()" [max]="segmentTotal()"></ui-progress-ring>
          </div>
        </ui-analytics-layout>

        <ui-activity-feed
          [title]="'Flux équipe'"
          [subtitle]="'Présences, retards et demandes'"
      ...
      <div layout-bottom>
        ...
        <section class="panel">
          ...
          @if ((data()?.people || []).length === 0) {
            <p class="empty">Aucun membre d'équipe affecté pour le moment.</p>
          } @else {
...
export class ManagerDashboardPageComponent {
  ...
  readonly data = this.store.managerData;
  readonly loading = this.store.isLoading('MANAGER');
  readonly error = this.store.getError('MANAGER');
  ...
  private loadData(showLoader: boolean, forceRefresh: boolean): void {
    this.store.loadDashboard('MANAGER', forceRefresh).pipe(take(1)).subscribe();
  }
