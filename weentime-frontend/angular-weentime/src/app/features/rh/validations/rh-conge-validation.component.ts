...
@Component({
  selector: 'app-rh-conge-validation',
  standalone: true,
    ...
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-yellow-500">
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-2xl font-bold text-gray-900">Validation des Congés</h2>
            <p class="text-gray-600 mt-2">Approbation finale des demandes de congés en attente</p>
          </div>
          <div class="text-right">
            <div class="text-4xl font-bold text-yellow-600">{{ rhValidationService.pendingByType().conges }}</div>
      ...
      @if (!rhValidationService.loadingSignal() && (rhValidationService.filteredPendingSignal() | slice:0:1).length === 0) {
        <div class="bg-white rounded-lg shadow-sm p-12 text-center border-2 border-dashed border-gray-300">
          ...
          <p class="text-gray-600">Toutes les demandes de congés ont été validées!</p>
        </div>
      }
      ...
      @if (!rhValidationService.loadingSignal()) {
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p class="text-blue-900 text-sm">
            ...
            Affichage des demandes de <strong>Congés</strong> en attente d'approbation RH (statut: EN_ATTENTE_RH)
          </p>
        </div>
      }
                  ...
                  <div class="flex items-start justify-between gap-4">
                    ...
                    <div class="flex-1">
                      ...
                      <div class="grid md:grid-cols-2 gap-4 text-sm mb-4">
                        ...
                        <div>
                          <span class="text-gray-600 font-medium">Nombre de jours:</span>
...
export class RhCongeValidationComponent implements OnInit {
  ...
  loadPendingValidations(): void {
    this.rhValidationService.loadPendingValidations('CONGE', 0, 50);
  }

  onValidateDemande(demande: RHValidationDemande): void {
    const commentaire = prompt(`Ajouter un commentaire pour approuver le congé de ${demande.utilisateur?.prenom}:`, '');
    
    if (commentaire !== null) {
      this.rhValidationService.validateDemande('CONGE', demande.id, commentaire).subscribe({
        next: () => {
          this.loadPendingValidations();
        },
        error: () => this.toastService.error("La validation du conge a echoue.")
      });
    }
  }
  ...
  onRejectDemande(demande: RHValidationDemande): void {
    ...
    if (commentaire !== null && commentaire.trim() !== '') {
      this.rhValidationService.rejectDemande('CONGE', demande.id, commentaire).subscribe({
        next: () => {
          this.loadPendingValidations();
        },
        error: () => this.toastService.error("Le refus du conge a echoue.")
      });
