...
def ensure_copilot_services(app_state: Any | None = None) -> dict[str, Any]:
    ...
    hr_policy_agent = getattr(state, "copilot_hr_policy_agent", None) or HRPolicyAgent()
...
async def process_copilot_message(
    ...
    language = provided_language if provided_language in {"fr", "en", "ar"} else detect_language(message)
